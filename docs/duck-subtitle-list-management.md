...
为倒鸭子模块添加了类似看板模块的字幕管理功能,用户现在可以手动添加、删除和编辑字幕,不再仅限于 SRT 文件导入。将原本分散的"字幕列表"和"字幕样式设置"两个面板合并为统一的"字幕管理"面板,提供更简洁直观的用户体验。

## 新增功能

### 1. 统一的字幕管理面板

在配置编辑器的右侧面板中添加了"字幕管理"折叠面板,位置在"字幕位置设定"下方。所有字幕的添加、删除、编辑功能都集中在这一个面板中。

**功能包括:**
- 显示当前字幕数量
- 添加新字幕按钮
- 字幕列表展示(可滚动,最大高度 320px)
- 每条字幕包含完整的编辑功能:文本、时间、样式、删除
...
原本的设计中有两个独立面板:
- **字幕列表管理** - 添加、删除字幕,编辑文本和时间
- **字幕样式设置** - 批量应用预设,调整字体、字号、动画

这种设计存在以下问题:
1. **功能分散** - 编辑一条字幕需要在两个面板间切换
2. **重复内容** - 两个面板都显示字幕列表,占用空间
3. **认知负担** - 用户需要理解两套系统(列表项 vs 样式索引)

### 合并后的优势

- **一站式编辑** - 所有属性集中在一个展开面板中
- **直观高效** - 点击字幕即可编辑所有相关属性
- **节省空间** - 单一列表,更大的可视区域(320px)
- **数据一致** - 字幕属性直接存储在 `DuckSubtitleItem` 中,无需额外映射
